...
struct MarkdownTextView: NSViewRepresentable {
  ...
  func makeNSView(context: Context) -> NSScrollView {
    let textView = ScealTextView(usingTextLayoutManager: true)
    ...
    context.coordinator.lastDividerCount = (textView as ScealTextView).sectionDividerCount
  ...
  func updateNSView(_ scrollView: NSScrollView, context: Context) {
    ...
    context.coordinator.lastAppliedAppearance = appearanceSettings
    context.coordinator.lastNoteID = noteID
    if let interactiveTV = textView as? ScealTextView {
  ...
  private func configure(_ textView: NSTextView, coordinator: Coordinator) {
    if let interactiveTextView = textView as? ScealTextView {
...
extension MarkdownTextView {
  ...
  final class Coordinator: NSObject, NSTextViewDelegate {
    ...
    private func handleListIndent(textView: NSTextView, increase: Bool) -> Bool {
      ...
      let appearance =
        (textView as? ScealTextView)?.appearanceSettings ?? .default
      return textView.performEditorEdit(
