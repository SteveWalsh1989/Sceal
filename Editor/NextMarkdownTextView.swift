...
// TextKit 2 editor shell with full editing behavior parity.
...
struct NextMarkdownTextView: NSViewRepresentable {
  ...
  func makeNSView(context: Context) -> NSScrollView {
    ...
    context.coordinator.lastDividerCount = (textView as NextInteractiveTextView).sectionDividerCount
  ...
  func updateNSView(_ scrollView: NSScrollView, context: Context) {
    ...
    context.coordinator.lastAppliedAppearance = appearanceSettings
    context.coordinator.lastNoteID = noteID
    if let interactiveTV = textView as? NextInteractiveTextView {
...
extension NextMarkdownTextView {
  ...
  final class Coordinator: NSObject, NSTextViewDelegate {
    ...
    func textViewDidChangeSelection(_ notification: Notification) {
      ...
      if let nextTV = textView as? NextInteractiveTextView,
    ...
    private func handleListIndent(textView: NSTextView, increase: Bool) -> Bool {
      ...
      let appearance =
        (textView as? NextInteractiveTextView)?.appearanceSettings ?? .default
      return textView.performEditorEdit(
    ...
    private func autoformatEditedLinesIfNeeded(
      ...
      if textView is NextInteractiveTextView {
    ...
    private func applySectionColorsToEditedLines(
      ...
      in textStorage: NSTextStorage,
      nextTextView: NextInteractiveTextView
    ) {
