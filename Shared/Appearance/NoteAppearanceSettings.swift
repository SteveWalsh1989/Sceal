...
struct NoteAppearanceSettings: Codable, Equatable, Sendable {
  ...
  var highlightsFocusedSectionBorder: Bool
  ...
  init(
    ...
    highlightsFocusedSectionBorder: Bool = true,
    ...
    self.highlightsFocusedSectionBorder = highlightsFocusedSectionBorder
  ...
  var clamped: NoteAppearanceSettings {
    NoteAppearanceSettings(
      ...
      highlightsFocusedSectionBorder: highlightsFocusedSectionBorder,
  ...
  var accentColor: NSColor {
    ThemePalette.color(named: normalizedAccentColorName)
      ?? ThemePalette.color(named: Self.defaultAccentColorName)
      ?? .systemPink
  }
  ...
  var resolvedColors: ThemeColorSet {
    ...
    return AppTheme.builtIn(id: themeID)?.colors
      ?? AppTheme.defaultDark.colors
  }

  // The color scheme the active theme requires.
  var preferredColorScheme: ColorScheme? {
    let theme = AppTheme.builtIn(id: themeID) ?? AppTheme.defaultDark
  ...
  private var normalizedAccentColorName: String {
    ThemePalette.colors.contains(where: { $0.name == accentColorName })
      ? accentColorName
  ...
  private enum CodingKeys: String, CodingKey {
    ...
    case highlightsFocusedSectionBorder
  ...
  init(from decoder: Decoder) throws {
    ...
    self.init(
      ...
      highlightsFocusedSectionBorder: try container.decodeIfPresent(
        Bool.self,
        forKey: .highlightsFocusedSectionBorder
      ) ?? true,
  ...
  func encode(to encoder: Encoder) throws {
    ...
    try container.encode(
      highlightsFocusedSectionBorder,
      forKey: .highlightsFocusedSectionBorder
    )
